(function() {
    var app = angular.module('forum-directives', ['djds4rce.angular-socialshare']);
    ...
    app.directive("post", function() {
        return {
            ...
            controller: ['$http', '$scope', '$sce', function($http, $scope, $sce) {
                ...
                $scope.htmlContent = $sce.trustAsHtml($scope.post.content.replace(/\n/g, '<br/>'));
                $scope.isLiked = false;
                $scope.flag = false; //show comment area flag

                $http.get("http://assc-klong-gh.azurewebsites.net/api/check_if_post_liked?postId=" + $scope.post.id + "&userId=" + $scope.userId).success(function(respondData) {
                ...
                $scope.showComment = function() {
                    if ($scope.flag == false) {
                        if ($scope.comments.length == 0) {
                            $http.get("https://assc-klong-gh.azurewebsites.net/tables/comment?$filter=postId eq '" + $scope.post.id + "'").success(function(data) {
                                $scope.comments = data;
                            });
                        }
                        $scope.flag = true;
                ...
                $scope.addLikes = function(post) {
                    ...
                    $http.get("http://assc-klong-gh.azurewebsites.net/api/check_if_post_liked?postId=" + $scope.post.id + "&userId=" + $scope.userId).success(function(respondData) {
                        if (!respondData.isLiked) {
                            ...
                            $http({ method: 'PATCH', url: url, data: angular.toJson(data) }).success(
                                function(data) {
                                    $http.get("http://assc-klong-gh.azurewebsites.net/api/set_post_liked_by_user?postId=" + post.id + "&userId=" + $scope.userId);
                ...
                $scope.hasComments = function(post) {
                    return $scope.comments !== undefined && $scope.comments !== null && $scope.comments.length !== 0;
                };
                ...
                $scope.addComment = function(post) {
                    ...
                    $http.post(
                        ...
                        $http.get("https://assc-klong-gh.azurewebsites.net/tables/comment?$filter=postId eq '" + post.id + "'").success(function(data) {
                            $scope.comments = data;
                            $scope.comment = {};
                        });
    ...
    app.directive("postComments", function() {
        return {
            ...
            controller: ['$http', '$scope', function($http, $scope) {
                ...
                $http.get('https://assc-klong-gh.azurewebsites.net/tables/comment/' + $scope.comment.id).success(function(data) {
                    var userId = data.userId;
                    $http.get('https://assc-klong-gh.azurewebsites.net/tables/user_profile/' + userId).success(function(data) {
                        $scope.user = data;
                    });
                });

                $http.get("http://assc-klong-gh.azurewebsites.net/api/check_if_comment_liked?commentId=" + $scope.comment.id + "&userId=" + $scope.currentUserId).success(function(respondData) {
                    if (respondData.isLiked)
                        $scope.isLiked = true;
                    else
                        $scope.isLiked = false;
                ...
                $scope.addLikes = function(comment) {
                    ...
                    $http.get("http://assc-klong-gh.azurewebsites.net/api/check_if_comment_liked?commentId=" + comment.id + "&userId=" + $scope.currentUserId).success(function(respondData) {
                        if (!respondData.isLiked) {
                            comment.likes++;
                            //Todo: post to server
                            var data = { "likes": comment.likes };
                            var url = "https://assc-klong-gh.azurewebsites.net/tables/comment/" + comment.id;

                            $http({ method: 'PATCH', url: url, data: angular.toJson(data) }).success(
                                function(data) {
                                    $http.get("http://assc-klong-gh.azurewebsites.net/api/set_comment_liked_by_user?commentId=" + comment.id + "&userId=" + $scope.currentUserId);
                ...
                $scope.addComment = function(comment) {
                    ...
                    $http.post(
                        ...
                        $http.get("https://assc-klong-gh.azurewebsites.net/tables/second_level_comment?$filter=commentId eq '" + comment.id + "'").success(function(data) {
                ...
                $scope.toggleCommentArea = function() {
                    if ($scope.commentAreaShowFlag == false) {
                        if ($scope.secondLevelComments.length == 0) {
                            $http.get("https://assc-klong-gh.azurewebsites.net/tables/second_level_comment?$filter=commentId eq '" + $scope.comment.id + "'").success(function(data) {
                                $scope.secondLevelComments = data;
                            });
                        }
    ...
    app.directive("secondLevelComments", function() {
        return {
            ...
            controller: ['$http', '$scope', function($http, $scope) {
                $scope.user = {};
                $http.get('https://assc-klong-gh.azurewebsites.net/tables/second_level_comment/' + $scope.secondLevelComment.id).success(function(data) {
                    var userId = data.userId;
                    $http.get('https://assc-klong-gh.azurewebsites.net/tables/user_profile/' + userId).success(function(data) {
                        $scope.user = data;
                    });
